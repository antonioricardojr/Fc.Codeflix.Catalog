...
public class GenreModelOutput
{
    ...
    public static GenreModelOutput FromGenre(Domain.Entity.Genre genre)
    {
        return new GenreModelOutput(genre.Id, genre.Name, genre.IsActive, genre.CreatedAt, genre.Categories.Select(categoryId => new GenreModelOutputCategory(categoryId)).ToList().AsReadOnly());
    }
}
